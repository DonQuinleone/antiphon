...
`PKGBUILD` and `.SRCINFO` here are the source for the `antiphon`
...
2. Copy this directory's `PKGBUILD` into the clone, or edit the
...
6. Build and check the package locally: `makepkg -si` (or
   `namcap PKGBUILD` and `namcap antiphon-$pkgver-$pkgrel-*.pkg.tar.zst`
   once built, to catch anything the guidelines require).
7. Commit `PKGBUILD` and `.SRCINFO` in the AUR clone and push:
   `git commit -m "..." && git push`.
8. Copy the same bump back into this repository's
   `dist/aur/PKGBUILD` and `dist/aur/.SRCINFO`, so they stay the
